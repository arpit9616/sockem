...
function readData(socket, data, key, opFunc, needBuffer) {
    if (!socket.__vssh_waiting_read) {
        ...
        if (data.length < bytesToRead) {
            ...
            opFunc(crypt.decrypt(dataSlice, key, needBuffer));
            readData(socket, dataNext, key, opFunc);
        } else opFunc(crypt.decrypt(data, key, needBuffer));
    } else {
        if (socket.__vssh_bytes_read + data.length == socket.__vssh_bytes_to_read) {
            ...
            opFunc(crypt.decrypt(new Buffer(socket.__vssh_data), key, needBuffer));
