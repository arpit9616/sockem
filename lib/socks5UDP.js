...
function handleSocksClient(tcpClient, server, rport, rhost) {
    ...
    server.on("message", (msg, rinfo) => {
        if (_isFromSocksClient(rinfo, tcpClient.address()) && _shouldWeRelay(msg, rport, rhost)) {
...
function _shouldWeRelay(chunk, rport, rhost) {
    ...
    const port = chunk.slice(addr.length+4,2).readInt16BE();
...
function _addClientHeader(destInfo, msg) {
    ...
    headeredMsg[0] = 0; headeredMsg[1] = 0; headeredMsg[2] = 0; headeredMsg[3] = utils.isThisAnIP(destInfo.address) ? (addr.length == 4 ? 0x01 : 0x04) : 0x03;
