...
function _handle_initial_client_conection(socket) {
    ...
    if (conf.clientTimeout) socket.setTimeout(conf.clientTimeout);
...
function _createSOCKS5Proxy(port, host, socket) {
    const conObj = extMan.preConnect(host, port); if (conf.remoteTimeout) conObj.timeout = conf.remoteTimeout;
...
function _handleSocksClient(client, remoteSocket, server, data, closeServer) {
    if (conf.clientTimeout) client.setTimeout(conf.clientTimeout);
    ...
    if (remoteSocket) remoteSocket.on("data", chunk => extMan.preIn(remoteSocket, chunk, chunkBack => client.write(chunkBack)));
