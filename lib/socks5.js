    ...
    const dataHandler = async chunk => { // all SOCKS5 commands should fit in a chunk
        try {
            ...
            if (messageNumber > 1) if (proxyObj) {  // Bad SOCKS5 client! Most probably libcurl nonsense, should be reconnecting to our new port
                ...
                _handleSocksClient(socket, proxyObj.remoteSocket, proxyObj.server, chunk, null, true);   
            }
        } catch (err) {
...
function _main(args) {  // allows standalone proxy operation, useful for non SOCKS5 clients (e.g. Windows "net use")
    if (args.length < 4) {
        console.log("Usage: socks5 <listening host> <listening port> <remote host> <remote port> <optional: timeout>");
        process.exit(1);
    }

    /* Override extensions config and init */
    CONSTANTS.EXTENSIONSCONF = CONSTANTS.CONFDIR+"/extensionsCmdLine.json";
	_doInit();

    /* Create command line proxy */
    const conObj = {host: args[2], port: args[3]}; if (args[4]) conObj.timeout = args[4];
    net.createServer({allowHalfOpen:true}, client => {
        const remoteSocket = net.connect(conObj, _ => _handleSocksClient(client, remoteSocket)); // resolve once remote is connected
    ...
    }).listen(args[1], args[0], _ => LOG.console(_log(`Created new proxy server listening on ${args[0]}:${args[1]}.`))
    ).on("error", err => LOG.error(_log(`Couldn't create a new proxy server, ${err}`)));
}
