...
function _handle_client_socket(socket) {
    ...
    const dataHandler = async chunk => { // all SOCKS5 commands should fit in a chunk
        try {
            ...
            if (messageNumber > 1) if (proxyObj) _handleClient(socket, proxyObj.remoteSocket, proxyObj.server, chunk, true);   // Bad SOCKS5 client! Most probably libcurl nonsense, should be reconnecting to our new port
        } catch (err) {
...
function _createSOCKS5Proxy(port, host, socket) {
    return new Promise((resolve, reject) => {
        ...
        const server = net.createServer({allowHalfOpen:true}, client => _handleClient(client, remoteSocket, server)
            ...
            LOG.debug(_log(`Created new proxy server listening on ${conf.proxy_host}:${server.address().port}.`, socket));
